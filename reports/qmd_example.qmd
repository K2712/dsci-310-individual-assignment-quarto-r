...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig:horse_pops width="90%"}

We can see from Figure [@fig:horse_pops] that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in Table [@tbl:horses_sd].

```{r}
#| label: tbl:horses_sd
#| tbl-cap: "Sample standard deviation of historical (1906-1972) horse populations for each Canadian province."
#| echo: false
#| warning: false
...
Additionally, note that in Table [@tbl:horses_sd] we consider the sample standard deviation of the number of horses during the same time span as Figure [@fig:horse_pops].

![Horse populations for the province with the largest standard deviation.](../results/horse_pop_plot_largest_sd.png){#fig:largest_sd width="65%"}

In Figure [@fig:largest_sd] we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
